...
export function FollowingPage() {
  ...
  const recommendedDesigners = useMemo(
    () => allDesigners.filter((d) => !followedDesignerIds.has(d.id)),
    [allDesigners, followedDesignerIds]
  );
  ...
  useEffect(() => {
    ...
    // Use shorter delay (0ms) when clearing filters for better UX
    const delay = (!isSearchActive && !selectedDesignerId && selectedFilters.length === 0) ? 0 : TIMING.SEARCH_DEBOUNCE;
  ...
  const handleDiscoverClick = () => {
    ...
    if (element) {
      ...
      // If element doesn't exist (all designers followed), show a message or do nothing
      console.warn("No recommended designers section found");
    }
  };
